...
public class TemplateFieldsResource extends AbstractRepoResource {
  ...
  @Operation(summary = "Resolve a template field identifier",
      description = "Return the template field a CEDAR identifier names. This is where an artifact's `@id` "
          + "dereferences to, so the path segment is the bare identifier that ends that IRI rather "
          + "than the IRI itself. Reading is checked twice: the caller needs the template field read "
  ...
  @ApiResponses({
      ...
      @ApiResponse(responseCode = "401", content = @Content(schema = @Schema(implementation = CedarError.class)),
          description = "The request carries no valid credentials, or the caller has no read access "
              ...
              + "than 403."),
      @ApiResponse(responseCode = "403", content = @Content(schema = @Schema(implementation = CedarError.class)), description = "The caller lacks the template field read permission"),
  ...
  public Response findTemplateField(
    ...
    if (userHasNoReadAccessToArtifact(c, fid)) {
      ...
      return CedarResponse.forbidden()
          .errorMessage("You do not have read access to this template field")
          .build();
    }
